	...
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="upload" v-if="type!='info'&& !ro.shangbaobaogao" label="上报报告" prop="shangbaobaogao">
					<file-upload
						tip="点击上传上报报告"
						action="file/upload"
						:limit="1"
						:multiple="true"
						:fileUrls="ruleForm.shangbaobaogao?ruleForm.shangbaobaogao:''"
						@change="shangbaobaogaoUploadChange"
					></file-upload>
				</el-form-item>  
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else-if="ruleForm.shangbaobaogao" label="上报报告" prop="shangbaobaogao">
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 15px","margin":"0 20px 0 0","outline":"none","color":"#000","borderRadius":"0","background":"url(http://codegen.caihongy.cn/20220727/34cf32af9b2e40ac9572071b85e16068.png) center center","width":"118px","lineHeight":"40px","fontSize":"14px","height":"43px"}' type="text" size="small" @click="download($base.url+ruleForm.shangbaobaogao)">下载</el-button>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else-if="!ruleForm.shangbaobaogao" label="上报报告" prop="shangbaobaogao">
...
export default {
	data() {
		...
		return {
			...
			rules: {
				...
				shangbaobaogao: [
					{ required: true, message: '上报报告不能为空', trigger: 'blur' },
				],
				sfsh: [
				],
				shhf: [
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='shangbaobaogao'){
							this.ruleForm.shangbaobaogao = obj[o];
							this.ro.shangbaobaogao = true;
							continue;
						}
				}
	...
	if(this.ruleForm.shangbaobaogao!=null) {
		this.ruleForm.shangbaobaogao = this.ruleForm.shangbaobaogao.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zhibudahui/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zhibudahuiCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    shangbaobaogaoUploadChange(fileUrls) {
	    this.ruleForm.shangbaobaogao = fileUrls;
    },
  }
};
</script>
