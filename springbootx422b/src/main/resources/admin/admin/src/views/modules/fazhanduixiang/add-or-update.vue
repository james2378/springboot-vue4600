	...
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="报告时间" prop="baogaoshijian">
					<el-date-picker
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.baogaoshijian" label="报告时间" prop="baogaoshijian">
					<el-input v-model="ruleForm.baogaoshijian" placeholder="报告时间" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="upload" v-if="type!='info'&& !ro.fazhanbaogao" label="发展报告" prop="fazhanbaogao">
					<file-upload
						tip="点击上传发展报告"
						action="file/upload"
						:limit="1"
						:multiple="true"
						:fileUrls="ruleForm.fazhanbaogao?ruleForm.fazhanbaogao:''"
						@change="fazhanbaogaoUploadChange"
					></file-upload>
				</el-form-item>  
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else-if="ruleForm.fazhanbaogao" label="发展报告" prop="fazhanbaogao">
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 15px","margin":"0 20px 0 0","outline":"none","color":"#000","borderRadius":"0","background":"url(http://codegen.caihongy.cn/20220727/34cf32af9b2e40ac9572071b85e16068.png) center center","width":"118px","lineHeight":"40px","fontSize":"14px","height":"43px"}' type="text" size="small" @click="download($base.url+ruleForm.fazhanbaogao)">下载</el-button>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else-if="!ruleForm.fazhanbaogao" label="发展报告" prop="fazhanbaogao">
...
export default {
	data() {
		...
		return {
			...
			rules: {
				...
				fazhanbaogao: [
					{ required: true, message: '发展报告不能为空', trigger: 'blur' },
				],
				sfsh: [
				],
				shhf: [
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='fazhanbaogao'){
							this.ruleForm.fazhanbaogao = obj[o];
							this.ro.fazhanbaogao = true;
							continue;
						}
				}
	...
	if(this.ruleForm.fazhanbaogao!=null) {
		this.ruleForm.fazhanbaogao = this.ruleForm.fazhanbaogao.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `fazhanduixiang/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.fazhanduixiangCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    fazhanbaogaoUploadChange(fileUrls) {
	    this.ruleForm.fazhanbaogao = fileUrls;
    },
  }
};
</script>
