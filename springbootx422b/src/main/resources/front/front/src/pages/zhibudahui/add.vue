...
<div :style='{"width":"1200px","padding":"20px","boxShadow":"0px 4px 10px 0px rgba(0,0,0,0.302)","margin":"10px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"80%","padding":"10px 9%","margin":"0 0 10px","background":"#fff"}' label="上报时间" prop="shangbaoshijian" >
              <el-date-picker
          ...
          <el-form-item :style='{"width":"80%","padding":"10px 9%","margin":"0 0 10px","background":"#fff"}' label="上报报告" prop="shangbaobaogao">
            <file-upload
            tip="点击上传上报报告"
            action="file/upload"
            :limit="1"
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('zhibudahui/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
                  ...
                  params: params
                     ...
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('zhibudahui/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
      ...
      shangbaobaogaoUploadChange(fileUrls) {
          this.ruleForm.shangbaobaogao = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");;
      },
    }
  };
</script>
