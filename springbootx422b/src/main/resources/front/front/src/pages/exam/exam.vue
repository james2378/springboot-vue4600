<template>
  <div>
    <div class="top-content">
      ...
      <div class="top-content-center" v-cloak>距离考核结束还有 <el-tag type="danger">{{this.hour}}</el-tag> 时 <el-tag type="danger">{{this.minute}}</el-tag> 分 <el-tag type="danger">{{this.second}}</el-tag> 秒</div>
      <div class="top-content-right">
        <el-button type="danger" @click="doneExam(0)">结束考核</el-button>
      </div>
    </div>
    <div class="question-content" v-for="(item, index) in questionList" :key="item.id" v-show="curQuesIndex == index">
      ...
      <div class="question-answer">
        ...
        <div v-show="end">
          <div class="question-title">考核成绩：<el-tag type="danger">{{totalScore}}</el-tag> 分</div>
        </div>
      </div>
      <div class="question-btn">
        ...
        <el-button v-show="end" type="success" @click="doneExam(1)">结束考核</el-button>
      </div>
    </div>
  </div>
</template>
...
<script>
  export default {
    ...
    methods: {
      ...
      doneExam(type) {
        if (type == 0) {
          this.$confirm('确定离开考核?未答题目按0分计', '提示', {
